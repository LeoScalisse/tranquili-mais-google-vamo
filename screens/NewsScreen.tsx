    ...
    return (
        <div className="bg-white h-full overflow-y-auto pb-28 px-4 py-8">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedItem(null)}>
                    <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6">
                            ...
                            {selectedItem.source_url && <a href={selectedItem.source_url} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 text-[#38b6ff] font-bold"><Globe className="w-5 h-5"/> Ler notícia completa</a>}
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex justify-between items-end mb-8">
                <div>
                    <Badge variant="secondary" className="mb-2 bg-blue-50 text-[#38b6ff]">Curadoria Digital</Badge>
                    <h1 className="text-3xl font-bold">Novidades <span className="text-[#ffde59]">+</span></h1>
                </div>
                <button onClick={() => fetchNews(selectedCategory)} className="p-2 bg-gray-100 rounded-full text-[#38b6ff]"><RefreshCwIcon className="w-6 h-6"/></button>
            </div>

            <div className="flex gap-2 mb-8 overflow-x-auto pb-2 no-scrollbar">
                {CATEGORIES.map(c => (
                    <button key={c.id} onClick={() => setSelectedCategory(c.id)} className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-bold border transition-all ${selectedCategory === c.id ? 'bg-[#38b6ff] text-white border-[#38b6ff]' : 'bg-white text-gray-500 border-gray-200'}`}>{c.label}</button>
                ))}
            </div>
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20 text-center animate-pulse">
                    ...
                    <p className="text-xl font-bold text-gray-700">Encontrando notícias fresquinhas para você...</p>
                    <p className="text-gray-400 mt-2">Nossa IA está conectando com os melhores portais.</p>
                </div>
            ) : (
                    ...
                    {newsItems.map((item, i) => (
                        <div key={i} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden cursor-pointer hover:shadow-lg transition-all group" onClick={() => { playSound('select'); setSelectedItem(item); }}>
                            ...
                            <div className="p-4">
                                ...
                                <h3 className="font-bold text-gray-800 group-hover:text-[#38b6ff] transition-colors">{item.title}</h3>
